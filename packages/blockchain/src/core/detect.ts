import type { ChainFilter, ChainId } from '@blocksense/shared';
...
export function detectInput(raw: string, hint: ChainFilter = 'all'): Detection {
  ...
  if (HEX_64.test(value)) {
    // A bare 64-character hex string is a valid Bitcoin txid and a valid TRON
    // hash, so both are offered. An explicit hint narrows the list as well as
    // the label — otherwise the label and the chains would disagree.
    const chains: ChainId[] =
      hint === 'bitcoin' ? ['bitcoin'] : hint === 'tron' ? ['tron'] : ['bitcoin', 'tron'];
    const label =
      hint === 'bitcoin' ? 'Bitcoin transaction detected' : hint === 'tron' ? 'TRON transaction detected' : 'Bitcoin or TRON transaction detected';
    return { kind: 'transaction', chains, label, action: 'transaction' };
  }
