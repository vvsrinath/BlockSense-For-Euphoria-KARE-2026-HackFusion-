...
function GraphNodeInner({ data, selected }: NodeProps<EntityNode>) {
  const { entity } = data;
  const meta = nodeKindMeta[entity.kind];
  const Icon = meta.icon;
  const isCenter = entity.kind === 'center';
  const StatusIcon = levelMeta[entity.level].icon;

  return (
    <div className="flex w-[120px] cursor-pointer flex-col items-center" aria-label={`${entity.label}, ${meta.label}, ${levelMeta[entity.level].label}`}>
      <div
        className={cn(
        ...
        <span className="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full bg-surface shadow-card">
            <StatusIcon className={cn('h-3 w-3', levelMeta[entity.level].text)} aria-hidden="true" />
          </span>
        }
