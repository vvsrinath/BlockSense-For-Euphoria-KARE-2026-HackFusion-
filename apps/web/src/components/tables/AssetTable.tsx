...
export function AssetTable({ assets }: AssetTableProps) {
  if (!assets.length) return <p className="py-8 text-center text-sm text-muted">No assets found.</p>;
      ...
      <table className="w-full min-w-[640px] text-left text-sm">
        ...
        <thead>
          <tr className="border-b border-line text-xs font-medium text-muted">
            <th scope="col" className="px-5 pb-2 font-medium md:px-6">Asset</th>
            <th scope="col" className="pb-2 font-medium">Symbol</th>
            <th scope="col" className="pb-2 font-medium">Chain</th>
            <th scope="col" className="pb-2 font-medium">Type</th>
            <th scope="col" className="pb-2 text-right font-medium">Holders</th>
            <th scope="col" className="px-5 pb-2 text-right font-medium md:px-6"><span className="sr-only">Action</span></th>
          </tr>
        </thead>
        <tbody>
          {assets.map((asset) => (
            <tr key={asset.id} className="border-b border-line last:border-b-0">
              ...
              <td className="py-3"><ChainBadge chain={asset.chain} /></td>
              ...
              <td className="px-5 py-3 text-right md:px-6">
                <Link
                  to={`/assets/${asset.id}`}
                  className="inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-[13px] font-medium text-primary hover:bg-primary/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 touch-target"
                  aria-label={`View ${asset.name} on ${asset.chain}`}
                >
                  View <ArrowRightIcon className="h-3.5 w-3.5" aria-hidden="true" />
                </Link>
              </td>
            </tr>
          ))}
